...
class Hcpb:
    ...
    def phi_ij(self, i, k, parity="cos"):
        """
        ...
        """
        if parity == "cos":
            cos_phi = (np.diag(self._off, k=1) + np.diag(self._off, k=-1)) / 2
            cos_ij = np.conj(self.evec_k(i)).dot(cos_phi.dot(self.evec_k(k)))
            # cos_ij = np.sum(cos_ij)
            return cos_ij
        else:
            sin_phi = (np.diag(self._off, k=1) - np.diag(self._off, k=-1)) / (2j)
            sin_ij = np.conj(self.evec_k(i)).dot(sin_phi.dot(self.evec_k(k)))
            # sin_ij = np.sum(sin_ij)
            return sin_ij
    ...
    def cosphi_to_qutip(self, n_transmon, thresh=1e-5):
        cos_op = np.zeros((n_transmon, n_transmon))
        for i in range(n_transmon):
            for j in range(n_transmon):
                val = self.phi_ij(i, j, parity="cos")
                if val < thresh:
                    val = 0
                cos_op[i, j] = val
        return qt.Qobj(cos_op)
